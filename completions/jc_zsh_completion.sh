...
_jc() {
    ...
    jc_options=(--force-color -C --debug -d --monochrome -m --ndjson-out -n --meta-out -M --pretty -p --quiet -q --raw -r --slurp -s --stream-buffer-limit -L --unbuffer -u --yaml-out -y)
    jc_options_describe=(
        ...
        '-m:monochrome output'
        '--ndjson-out:NDJSON output (one JSON object per line)'
        '-n:NDJSON output (one JSON object per line)'
        ...
        '--stream-buffer-limit:set max items for non-NDJSON streaming (0=unlimited)'
        '-L:set max items for non-NDJSON streaming (0=unlimited)'
    ...
    jc_about_mod_options=(--pretty -p --yaml-out -y --ndjson-out -n --monochrome -m --force-color -C --stream-buffer-limit)
