...
from jc.cli_args import CliParseResult, MagicParseResult, SlicerOptions, ParserOptions, OutputOptions
...
class MyTests(unittest.TestCase):
    def test_cli_magic_parser(self):
        ...
        for command, expected in commands.items():
            result = CliParseResult._parse_magic_syntax(command.split())
            resulting_attributes = (result.found_parser, result.options, result.run_command)
    ...
    def test_cli_set_env_colors(self):
        ...
        for jc_colors, expected_colors in env.items():
            ...
            self.assertEqual(cli.runtime_config.custom_colors, expected_colors)
    ...
    def test_cli_json_out_pretty(self):
        ...
        for test_dict, expected_json in zip(test_input, expected_output):
            cli = JcCli()
            cli.output_opts = OutputOptions(pretty=True)
            cli.set_custom_colors()
            cli.data_out = test_dict
            self.assertEqual(cli.json_out(), expected_json)
    ...
    def test_add_meta_to_simple_dict(self):
        ...
        cli.parse_result = CliParseResult(
            magic=MagicParseResult(run_command=['ping', '-c3', '192.168.1.123'])
        )
    ...
    def test_slice_none_str(self):
        cli = JcCli()
        cli.parse_result = CliParseResult(
            slicer=SlicerOptions(slice_start=None, slice_end=None)
        )
        cli.data_in = '''\
        row0
        row1
    ...
    def test_slice_positive_str(self):
        cli = JcCli()
        cli.parse_result = CliParseResult(
            slicer=SlicerOptions(slice_start=1, slice_end=5)
        )
        cli.data_in = '''\
        row0
        row1
    ...
    def test_slice_negative_str(self):
        cli = JcCli()
        cli.parse_result = CliParseResult(
            slicer=SlicerOptions(slice_start=1, slice_end=-1)
        )
        cli.data_in = '''\
        row0
        row1
    ...
    def test_slurp_standard_with_meta_out(self):
        cli = JcCli()
        cli.parse_result = CliParseResult(
            parser_opts=ParserOptions(meta_out=True)
        )
        cli.parser_module = url_parser
        cli.data_in = '''http://www.google.com
            https://www.kelly.com/testing
    ...
    def test_slurp_proc(self):
        cli = JcCli()
        cli.parse_result = CliParseResult(
            parser_opts=ParserOptions(quiet=True)
        )
...
'''
    ...
    def test_slurp_proc_with_meta_out(self):
        cli = JcCli()
        cli.parse_result = CliParseResult(
            parser_opts=ParserOptions(meta_out=True, quiet=True),
            magic=MagicParseResult(run_command=['/proc/stat', '/proc/cpuinfo'])
        )
        cli.parser_module = proc_parser
